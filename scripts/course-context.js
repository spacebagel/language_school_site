...
function renderCourses(courses) {
    ...
    paginatedCourses.forEach(course => {
        ...
        courseElement.innerHTML = `
            ...
            <button onclick='getCourseRequest(${JSON.stringify(course)})'>Get Course</button>
        `;
...
function getCourseRequest(course) {
    const modalHtml = `
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content course">
                    <div class="modal-header">
                        <h5 class="modal-title" id="courseRequestLabel">Course Request</h5>
                        ...
                        <form id="courseRequestForm">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="startDate">
                                    ...
                                    ${course.start_dates.map(date => `<option value="${date}">${new Date(date).toLocaleDateString()}</option>`).join('')}
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="startTime" class="form-label">Start Time</label>
                                <select class="form-select" id="startTime" disabled>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="numStudents" min="1" max="20" value="1">
                            </div>
                            <div class="mb-3">
                                <label>Additional Options</label>
                                <div>
                                    <input type="checkbox" id="supplementary" value="2000">
                                    <label for="supplementary">Supply Materials (+2000 per student)</label>
                                </div>
                                <div>
                                    <input type="checkbox" id="personalized" value="1500">
                                    <label for="personalized">Personalized Sessions (+1500 per week)</label>
                                </div>
                                <div>
                                    <input type="checkbox" id="excursions" value="0.25">
                                    <label for="excursions">Cultural Excursions (+25%)</label>
                                </div>
                                <div>
                                    <input type="checkbox" id="assessment" value="300">
                                    <label for="assessment">Language Assessment (+300)</label>
                                </div>
                                <div>
                                    <input type="checkbox" id="interactive" value="1.5">
                            ...
                            <div class="container text-center">
                                <button type="submit" class="main-wave-button">Submit</button>
    ...
    async function formCourseSubmitHandler(e) {
        ...
        try {
            const response = await fetch(orderPostUrl, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                let modalElement = document.getElementById('courseRequestModal');
                let modalInstance = bootstrap.Modal.getInstance(modalElement);
                modalInstance.hide();
                e.target.reset();
            } else {
                console.error('Order POST error message:', response.status);
            }
...
function populateTimes(course, selectedDate, weekLength) {
    ...
    if (selectedDate) {
        const times = course.start_dates.filter(date => date.startsWith(selectedDate)).map(date => {
            ...
            const endTime = new Date(new Date(date).getTime() + weekLength * 60 * 60 * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
